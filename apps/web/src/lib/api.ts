...
export const endpoints = {
  ...
  analytics: (range: string = '7d') => apiFetch(`/api/endpoints/analytics?range=${range}`),
...
export const settings = {
  getSupport: () => apiFetch('/api/settings/support'),
...
export const admin = {
  ...
  updateSetting: (key: string, value: any) =>
    apiFetch(`/api/admin/settings/${key}`, { method: 'PUT', body: JSON.stringify({ value }) }),
};
