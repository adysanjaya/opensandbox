...
import { endpoints, users, sharedFunctions, apiLogs } from '../db/schema.js';
...
app.all('/:userHash/:slug', async (c) => {
  ...
  const logExecution = (statusCode: number) => {
    ...
    db.insert(apiLogs)
      ...
      .catch((err) => console.error('Failed to log API execution:', err));
  };
